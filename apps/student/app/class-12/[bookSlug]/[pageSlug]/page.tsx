 ...
 * at /class-12/chemistry). Serving them here too would publish the same pages
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const book = await BookModel
    .findOne({ slug: bookSlug, grade: 12, is_published: true })
    .lean<Book | null>();
  if (!book) return {};
  ...
  return buildBookPageMetadata({ book, page, basePath: '/class-12' });
}

export default async function Class12PageRoute({ params }: Props) {
  const { bookSlug, pageSlug } = await params;
  if (PINNED_ELSEWHERE.has(bookSlug)) notFound();

  await connectToDatabase();

  const book = await BookModel.findOne({ slug: bookSlug, grade: 12 }).lean<Book | null>();
