...
function BenzeneSVG({ slot, slotIdx, onSetPos }: { slot: SimSlot; slotIdx: number; onSetPos: (p: Position) => void }) {
  ...
  return (
    <svg viewBox="0 0 140 160" width="140" height="150" style={{ overflow: 'visible', display: 'block', margin: '0 auto' }}>
      ...
      <text x={v0x.toFixed(1)} y={(v0y - 24).toFixed(1)} textAnchor="middle" fontSize="14" fontWeight="700" fill="rgba(255,255,255,0.9)">OH</text>
      {slot.group !== 'H' && (<>
        ...
        <text x={(vsx + ux * 32).toFixed(1)} y={(vsy + uy * 32 + 4).toFixed(1)} textAnchor={ta} fontSize="14" fontWeight="700" fill={col}>{g.sym}</text>
      </>)}
      {positions.map(p => {
...
function SlotTile({ slot, slotIdx, isActive, onActivate, onSetPos }: { slot: SimSlot; slotIdx: number; isActive: boolean; onActivate: () => void; onSetPos: (p: Position) => void }) {
  ...
  return (
    <div onClick={onActivate} style={{ borderRadius: 10, padding: '11px 11px 9px', background: isActive ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.03)', border: `1px solid ${isActive ? col : 'rgba(255,255,255,0.08)'}`, cursor: 'pointer', transition: 'all .2s' }}>
      <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase' as const, color: isActive ? col : 'rgba(255,255,255,0.3)', marginBottom: 6 }}>Compound {slotIdx + 1}</div>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 6, marginBottom: 4 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'rgba(255,255,255,0.92)', lineHeight: 1.1, marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{simLabel(slot)}</div>
        ...
        <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
          ...
          <div className="font-mono" style={{ fontSize: 20, fontWeight: 800, color: isActive ? col : 'rgba(255,255,255,0.6)', lineHeight: 1 }}>{pka.toFixed(2)}</div>
        </div>
      </div>
        ...
        {(['ortho', 'meta', 'para'] as Position[]).map(p => {
          ...
          return (
            <button key={p} onClick={e => { e.stopPropagation(); onSetPos(p); }}
              style={{
                flex: 1, padding: '5px 0', borderRadius: 5, fontSize: 12.5, fontWeight: isOn ? 700 : 500, cursor: 'pointer', letterSpacing: '.02em',
      ...
      <div style={{ height: 18 }}>
        {delta !== 0 && <div style={{ textAlign: 'center' as const, fontSize: 13.5, color: delta < 0 ? '#4ade80' : '#fb7185', marginTop: 6, fontWeight: 700 }}>{delta > 0 ? '+' : ''}{delta.toFixed(2)} vs PhOH</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ fontSize: 12.5, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.35)' }}>
          Substituent &nbsp;·&nbsp; <span style={{ color: '#fb923c' }}>● EWG</span> &nbsp; <span style={{ color: '#10b981' }}>● EDG</span> &nbsp; <span style={{ color: '#6366f1' }}>● Neutral</span>
        </div>
        <div style={{ fontSize: 11, fontWeight: 800, padding: '3px 10px', borderRadius: 6, background: 'rgba(139,92,246,0.12)', color: '#a78bfa', border: '1px solid rgba(139,92,246,0.25)', letterSpacing: '.05em', textTransform: 'uppercase' }}>
          5.3M+ Combinations
        </div>
      </div>
        ...
        {GROUPS.map(g => {
          ...
          return (
            <button key={g.id} onClick={() => onSelect(g.id)}
              style={{
                padding: '6px 12px', borderRadius: 8, fontSize: 12.5, cursor: 'pointer', fontWeight: isSel ? 700 : 500, letterSpacing: '.02em',
              ...
              onMouseEnter={e => {
                if (!isSel) {
                  ...
                  e.currentTarget.style.color = ts.selCol;
                }
              }}
              onMouseLeave={e => {
                if (!isSel) {
                  e.currentTarget.style.background = 'rgba(255,255,255,0.03)';
                  e.currentTarget.style.color = ts.col;
                }
              }}
            >
              {g.sym}
...
function PkaNumberLine({ slots }: { slots: SimSlot[] }) {
  ...
  for (let v = Math.ceil(lo); v <= Math.floor(hi); v++) {
    ...
    ticks.push(<g key={v}><line x1={x} y1={ty - 4} x2={x} y2={ty + 4} stroke="rgba(255,255,255,0.15)" strokeWidth="1" /><text x={x} y={ty + 17} textAnchor="middle" fontSize="10.5" fill="rgba(255,255,255,0.3)">{v}</text></g>);
  }
  return (
    <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" style={{ overflow: 'visible' }}>
        ...
        {pkas.map((p, idx) => {
          ...
          return <g key={idx}><line x1={cx} y1={ty} x2={cx} y2={cy} stroke={p.col} strokeWidth="1.5" opacity={0.4} /><circle cx={cx} cy={cy} r="10" fill={p.col} opacity={0.9} /><text x={cx} y={(+cy + 3.8).toFixed(1)} textAnchor="middle" fontSize="10.5" fontWeight="700" fill="white">{p.i + 1}</text></g>;
        })}
      </svg>
    </div>
  );
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', marginTop: 20, paddingTop: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' as const }}>
        <div>
          <div style={{ fontSize: 27, fontWeight: 800, lineHeight: 1, color: col }}>{g.sym}</div>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' as const, marginTop: 4, color: typeCol }}>{typeLabel}</div>
        </div>
        ...
        <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
          <div style={{ fontSize: 13.5, letterSpacing: '.12em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.25)', marginBottom: 2 }}>pKₐ</div>
          <div className="font-mono" style={{ fontSize: 42, fontWeight: 800, lineHeight: 1, color: col }}>{pka.toFixed(2)}</div>
          <div style={{ fontSize: 12.5, marginTop: 8, color: deltaRaw < 0 ? '#10b981' : '#fb923c', fontWeight: 600 }}>Δ {deltaRaw >= 0 ? '+' : ''}{deltaRaw.toFixed(2)} vs Phenol</div>
        </div>
      </div>
        ...
        {(['ortho', 'meta', 'para'] as const).map(p => {
          ...
          return (
            <div key={p} style={{ flex: 1, padding: '7px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' as const }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.08em', color: 'rgba(255,255,255,0.25)', marginBottom: 3 }}>Hammett σ {p}</div>
              <div className="font-mono" style={{ fontSize: 15, fontWeight: 700, color: sc }}>{sv >= 0 ? '+' : ''}{sv.toFixed(2)}</div>
            </div>
          );
        })}
      </div>
