// Export sidebar for the Structure Editor (local build — no cloud upload).
//   • Theme: "Light/print" = black lines; "Dark page" = white lines on a
//     transparent background (so a structure drops cleanly onto a dark page).
//     Indigo (the export engine) can only draw black, so dark mode is produced by
//     recolouring the exported SVG black→white in code.
//   • Download SVG (vector) / Download PNG (rasterised at 2x) / Copy PNG.
...
export default function ExportToolbar({
  ...
  // Recolour the exported SVG by INJECTING a <style> block (Indigo's exact output
  // — hex vs rgb vs named — varies, so regex replacement is unreliable). CSS with
  // !important overrides whatever Indigo emitted. Both themes drop the opaque
  // background rect → transparent; dark forces every bond/label to white.
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      ...
      <p className="mt-1 text-[11px] leading-relaxed text-white/30">
        ...
        background; &ldquo;Light/print&rdquo; gives black lines. Files save to your
        Downloads folder.
      </p>
    </div>
  );
