...
function heatBg(v: number | null, sel: boolean): string {
  ...
  // Softer Diverging Palette (Muted Red -> Soft Orange -> Muted Blue)
  if (t < 0.5) {
    const s = t / 0.5;
    r = Math.round(235 + s * 20);     // 235 -> 255
    g = Math.round(95 + s * 95);      // 95 -> 190
    b = Math.round(95 - s * 55);      // 95 -> 40
  } else {
    const s = (t - 0.5) / 0.5;
    r = Math.round(255 - s * 175);    // 255 -> 80
    g = Math.round(190 - s * 30);     // 190 -> 160
    b = Math.round(40 + s * 160);     // 40 -> 200
  }
...
function MiniRingSVG({ sub, pos, col, pka }: { sub: string; pos: 'ortho' | 'meta' | 'para'; col: string; pka: string }): React.ReactElement {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ overflow: 'visible' }}>
      ...
      <text x={v0x.toFixed(1)} y={(v0y - 32).toFixed(1)} textAnchor="middle" fontSize="16" fontWeight="700" fill="rgba(255,255,255,0.88)">COOH</text>
      ...
      <text x={lx.toFixed(1)} y={ly.toFixed(1)} textAnchor={ta} dominantBaseline="middle" fontSize="17" fontWeight="700" fill={col}>{sub}</text>
      <text x={pkaX.toFixed(1)} y={(cy + r + 22).toFixed(1)} textAnchor={pkaA} fontSize="16" fontWeight="800" fill="rgba(255,255,255,0.92)" fontFamily="var(--font-geist-mono),monospace">{pka}</text>
    </svg>
  );
}
    ...
    return (
      <div>
        <svg width="240" height="100" viewBox="0 0 240 100" style={{ overflow: 'visible', display: 'block', margin: '0 auto 10px' }}>
          <text x="20" y="52" fontSize="17" fontWeight="700" fill={typeCol} dominantBaseline="middle">{row.y}</text>
          <line x1={xOff} y1="52" x2={xOff + 18} y2="52" stroke="rgba(255,255,255,0.5)" strokeWidth="2" />
          <text x={xOff + 22} y="52" fontSize="16" fill="rgba(255,255,255,0.75)" dominantBaseline="middle">CH₂</text>
          <line x1={xOff + 58} y1="52" x2={xOff + 76} y2="52" stroke="rgba(255,255,255,0.5)" strokeWidth="2" />
          <text x={xOff + 80} y="52" fontSize="16" fontWeight="600" fill="rgba(255,255,255,0.9)" dominantBaseline="middle">COOH</text>
          <text x="120" y="85" textAnchor="middle" fontSize="13" fill="rgba(255,255,255,0.28)" fontFamily="var(--font-outfit),sans-serif">inductive only — no resonance</text>
        </svg>
        <div style={{ textAlign: 'center' as const }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, color: typeCol, marginBottom: 8, letterSpacing: '.04em' }}>{posLabel} · {row.y}</div>
          <div style={{ display: 'inline-flex', alignItems: 'baseline', gap: 10, justifyContent: 'center' }}>
            <span style={{ fontSize: 19, fontWeight: 600, color: 'rgba(255,255,255,0.45)' }}>pKₐ =</span>
            <span style={{ fontSize: 32, fontWeight: 800, fontFamily: 'var(--font-geist-mono),monospace', color: '#fff', lineHeight: 1 }}>{v !== null ? (v as number).toFixed(2) : '—'}</span>
          </div>
          <div style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.42)', marginTop: 12, lineHeight: 1.65, textAlign: 'left' as const }}>{mech}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <svg width="240" height="195" viewBox="0 0 240 195" style={{ overflow: 'visible', display: 'block', margin: '0 auto 10px' }}>
        ...
        <text x={v0x.toFixed(1)} y={(v0y - 44).toFixed(1)} textAnchor="middle" fontSize="17" fontWeight="700" fill="rgba(255,255,255,0.92)">COOH</text>
        ...
        <text x={lx.toFixed(1)} y={ly.toFixed(1)} textAnchor={ta} dominantBaseline="middle" fontSize="17" fontWeight="700" fill={typeCol}>{row.y}</text>
      </svg>
      <div style={{ textAlign: 'center' as const }}>
        <div style={{ fontSize: 13.5, fontWeight: 700, color: typeCol, marginBottom: 8, letterSpacing: '.04em' }}>{posLabel} · {row.y}</div>
        <div style={{ display: 'inline-flex', alignItems: 'baseline', gap: 10, justifyContent: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' as const, marginBottom: 20 }}>
        {[
          ...
          <div key={item.label} style={{ flex: 1, minWidth: 150, padding: '14px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: `1px solid ${item.col}30`, textAlign: 'center' as const }}>
            ...
            <div style={{ fontSize: 30, fontWeight: 800, color: item.col, lineHeight: 1 }}>pKₐ₁ <span style={{ fontFamily: 'var(--font-geist-mono),monospace' }}>{item.pka}</span></div>
        ...
        {[
          ...
          <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ textAlign: 'center' as const, padding: '12px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: `1px solid ${item.col}30`, minWidth: 110 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, fontFamily: 'var(--font-geist-mono),monospace', color: '#fff' }}>{item.pka}</div>
      ...
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' as const, marginBottom: 20 }}>
        {[
          ...
          <div key={item.label} style={{ flex: 1, minWidth: 180, padding: '14px 16px', borderRadius: 12, background: `rgba(255,255,255,0.03)`, border: `1px solid ${item.col}25` }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '.08em', color: item.col, marginBottom: 8 }}>{item.label}</div>
            <div style={{ fontSize: 20, fontWeight: 800, fontFamily: 'var(--font-geist-mono),monospace', color: item.col, marginBottom: 6 }}>{item.pka}</div>
...
function ActiveMethylenePkaTable() {
  ...
  const PKA_DATA_LEFT = [
    {
      type: '[1] Amide', pka: '30', svg: (
        <svg width="120" height="80" viewBox="0 0 120 80" style={{ overflow: 'visible' }}>
          ...
          <text x="55" y="8" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
          <line x1="55" y1="25" x2="72" y2="40" stroke={bond} strokeWidth="1.5" />
          <text x="76" y="47" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">N</text>
    ...
    },
    {
      type: '[2] Nitrile', pka: '25', svg: (
        <svg width="120" height="80" viewBox="0 0 120 80" style={{ overflow: 'visible' }}>
          ...
          <text x="68" y="27" fill={textFill} fontSize="14" fontFamily="var(--font-geist-mono),monospace">CN</text>
        </svg>
      )
    },
    {
      type: '[3] Ester', pka: '25', svg: (
        <svg width="120" height="80" viewBox="0 0 120 80" style={{ overflow: 'visible' }}>
          ...
          <text x="45" y="8" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
          <line x1="45" y1="25" x2="60" y2="40" stroke={bond} strokeWidth="1.5" />
          <text x="65" y="48" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
    ...
    },
    {
      type: '[5] Aldehyde', pka: '17', svg: (
        <svg width="120" height="80" viewBox="0 0 120 80" style={{ overflow: 'visible' }}>
          ...
          <text x="73" y="48" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">H</text>
        </svg>
      )
    }
  ];
  ...
  const PKA_DATA_RIGHT = [
    {
      type: '[6] 1,3-Diester', pka: '13.3', svg: (
        <svg width="180" height="80" viewBox="0 0 180 80" style={{ overflow: 'visible' }}>
          ...
          <text x="42" y="52" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
          ...
          <text x="60" y="15" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
          ...
          <text x="100" y="15" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
          <line x1="100" y1="35" x2="111" y2="45" stroke={bond} strokeWidth="1.5" />
          <text x="118" y="52" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
    ...
    },
    {
      type: '[7] 1,3-Dinitrile', pka: '11', svg: (
        <svg width="180" height="80" viewBox="0 0 180 80" style={{ overflow: 'visible' }}>
          <text x="40" y="45" fill={textFill} fontSize="14" textAnchor="end" fontFamily="var(--font-geist-mono),monospace">NC</text>
          <line x1="45" y1="40" x2="65" y2="50" stroke={bond} strokeWidth="1.5" />
          <line x1="65" y1="50" x2="85" y2="40" stroke={bond} strokeWidth="1.5" />
          <text x="90" y="45" fill={textFill} fontSize="14" textAnchor="start" fontFamily="var(--font-geist-mono),monospace">CN</text>
    ...
    },
    {
      type: '[8] β-Keto ester', pka: '10.7', svg: (
        <svg width="180" height="80" viewBox="0 0 180 80" style={{ overflow: 'visible' }}>
          ...
          <text x="40" y="15" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
          ...
          <text x="80" y="15" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
          <line x1="80" y1="35" x2="91" y2="45" stroke={bond} strokeWidth="1.5" />
          <text x="98" y="52" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
    ...
    },
    {
      type: '[9] β-Diketone', pka: '9', svg: (
        <svg width="180" height="80" viewBox="0 0 180 80" style={{ overflow: 'visible' }}>
          ...
          <text x="50" y="15" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
          ...
          <text x="90" y="15" fill={textFill} fontSize="14" textAnchor="middle" fontFamily="var(--font-geist-mono),monospace">O</text>
  ...
  const TableRow = ({ item }: { item: TableRowItem }) => (
    <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.04)', padding: '8px', fontSize: 13 }}>
      ...
      <div style={{ width: '27%', textAlign: 'right', fontFamily: 'var(--font-geist-mono),monospace', fontWeight: 700, color: '#34d399', fontSize: 15, paddingRight: 8 }}>{item.pka}</div>
    </div>
  );
