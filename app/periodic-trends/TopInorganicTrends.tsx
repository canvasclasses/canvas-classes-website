            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center mb-12">
                    ...
                    <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
                        Top 50 Trends of <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-yellow-500">Inorganic Chemistry</span>
                    </h2>
                    ...
                    {inorganicTrendsData.map((section, index) => (
                        <div key={index} className="scroll-mt-24" id={`section-${index}`}>
                            ...
                            <div className="flex items-center gap-4 mb-8">
                                <span className="flex-shrink-0 w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center text-lg font-bold text-gray-400 border border-gray-700">
                                    {index + 1}
                                </span>
                                <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {section.blocks.map((block) => (
                                    <motion.div
                                        ...
                                        key={block.id}
                                        className="bg-[#161b22] rounded-xl border border-gray-800 hover:border-gray-600 transition-all duration-300 flex flex-col h-full group"
                                    >
                                        <div className="p-5 flex-grow">
                                            {/* Block Header */}
                                            <div className="flex justify-between items-start mb-4">
                                                <h4 className="font-semibold text-gray-200 text-lg group-hover:text-amber-400 transition-colors">
                                                    {block.title}
                                                </h4>
                                                <span className="text-xs font-mono text-gray-600 bg-gray-900 px-2 py-1 rounded border border-gray-800">
                                                    #{block.id}
                                                </span>
                                            </div>

                                            {/* Trend Visual */}
                                            <div className="mb-6 p-4 bg-gray-900/50 rounded-lg border border-gray-800/50">
                                                <div className="text-center font-mono text-lg md:text-xl text-cyan-300 overflow-x-auto whitespace-nowrap scrollbar-thin scrollbar-thumb-gray-700 pb-1">
                                                    <ReactMarkdown
