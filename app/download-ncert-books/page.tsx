...
import { Book, FileText, Beaker, Download, X, ExternalLink, BookOpen, Loader2 } from 'lucide-react';
import { fetchNcertBooksData, getChapters, NcertChapter, NcertBooksData } from '@/app/lib/ncertBooksData';
...
export default function NcertBooksPage() {
    ...
    const [data, setData] = useState<NcertBooksData | null>(null);
    const [loading, setLoading] = useState(true);

    // Fetch data on mount
    useEffect(() => {
        const loadData = async () => {
            const fetchedData = await fetchNcertBooksData();
            setData(fetchedData);
            setLoading(false);
        };
        loadData();
    ...
    const chapters11 = data ? getChapters(data, '11', activeTab) : [];
    const chapters12 = data ? getChapters(data, '12', activeTab) : [];
