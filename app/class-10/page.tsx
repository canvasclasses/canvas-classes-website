...
export default async function Class10Page() {
  ...
  const rawBooks = await BookModel.find({ grade: 10, is_published: true })
    .sort({ subject: 1, title: 1 })
    .lean();

  // No published books yet — show the coming soon page
  if (rawBooks.length === 0) {
    return <LiveBooksComingSoon grade={10} expectedSubjects={EXPECTED_SUBJECTS} />;
  }
  ...
  return <GradeLandingPage grade={10} books={books} pages={pages} basePath="/class-10" />;
}
