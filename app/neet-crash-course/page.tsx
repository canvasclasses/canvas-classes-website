...
import { fetchNeetCrashCourseData, NeetChapter } from '@/app/lib/neetCrashCourseData';

export default function NeetCrashCoursePage() {
    const [chapters, setChapters] = useState<NeetChapter[]>([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        async function loadData() {
            const data = await fetchNeetCrashCourseData();
            setChapters(data);
            setLoading(false);
        }
        loadData();
