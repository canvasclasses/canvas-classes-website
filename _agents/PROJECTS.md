...
|---|---|---|---|---|---|
| **Adaptive Persona Engine** | 🟡 | Phase 1 core shipped (skill graph · Books→persona · recommender live · mastery-prob + spaced-rep + event log) | 2026-06-06 | When lecture/audio content exists: wire modality signal + finish audit #5 (test/batch profile write); then Phase 2 (AI tutor) | **⛔ Content:** lecture/video + audio-note content, and `ResourceLink` rows for skills beyond English grammar (8 exist, grammar-only). Engine works now but has little to recommend until content lands. |
...
- **Adaptive Persona Engine → needs content.** The engine (mastery model, spaced repetition, recommender, event log) is built and verified, but it can only recommend "read/watch this" once there are **lecture/audio resources + `ResourceLink` rows** for the skills students are weak on. Today: 8 ResourceLinks, English-grammar only. **When you build & upload lectures/audio:** (1) add `ResourceLink` rows (skill → resource) — see `seed_resource_links.js` for the pattern; (2) emit `learning_events` from the lecture/flashcard surfaces; (3) the modality-preference signal then becomes computable. Full detail: `_agents/plans/UNIFIED_LEARNER_PERSONA.md`.
...
|---|---|
| Adaptive Persona Engine | `_agents/plans/UNIFIED_LEARNER_PERSONA.md` + ADR-011 + GBrain `decisions/2026-06-06-unified-learner-persona.md` |
