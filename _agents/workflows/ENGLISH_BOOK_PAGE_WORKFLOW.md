...
| Block types `text`, `heading`, `image`, `callout`, `comparison_card`, `timeline`, `table`, `inline_quiz`, `worked_example`, `curiosity_prompt`, `reasoning_prompt`, `meet_a_scientist`, `section` | Inherited and reused. `meet_a_scientist` is reused **as is** but populated with authors. See §3.2 below for the mapping. `table` body cells render at body-paragraph size (`text-[17px]`) — never author or introduce a smaller-print table; see BOOK_PAGE_WORKFLOW §3.8. |
