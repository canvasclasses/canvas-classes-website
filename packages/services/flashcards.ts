...
export async function GET(req: NextRequest, deps: ServiceDeps) {
  try {
    await connectToDatabase();

    // Check authentication (safe localhost bypass only)
    const user = await deps.getAuthenticatedUser(req);
    const isAuthenticated = (await deps.isLocalhostDev()) || !!user;
...
export async function POST(req: NextRequest, deps: ServiceDeps) {
  try {
    await connectToDatabase();
