...
// Renders a `motion_lab` block: an optional predict-first gate, then the E2
// lab (Projectile Playground / Circular Arena, chosen by `scenario`), with
// title/caption.
//
// The lab is loaded ssr:false — it is pointer-event driven and client-only, the
// same discipline VectorBoardRenderer, MathGraphRenderer and the simulation
// registry follow. See motion-lab/archetypes.ts for the construction library
// and PHYSICS_SIMULATION_PROGRAM.md §5.3–5.4 for the pedagogy.
...
const MotionLab = dynamic(() => import('./motion-lab/MotionLab'), {
  ssr: false,
  loading: () => (
    <div
      className="w-full animate-pulse rounded-2xl border"
      style={{ height: 460, background: SIM_SURFACE, borderColor: BORDER.card }}
    />
  ),
});

export default function MotionLabRenderer({ block }: { block: MotionBenchBlock }) {
      ...
      {gated ? (
        <div className="rounded-xl border p-4" style={{ background: 'var(--book-surface, #181A21)', borderColor: BORDER.card }}>
          ...
          <p className="mt-3 text-[11px]" style={{ color: TEXT.ghost }}>
            Commit a guess — then the lab unlocks and you can test it.
          </p>
        </div>
      ) : (
        <>
          <MotionLab block={block} />
          {predict && picked !== null && (
            <div
