...
// Renders a `field_bench` block: an optional predict-first gate, then the E5
// bench (field sculptor / Gauss surface / trajectory / photoelectric, chosen by
// `kind` × `mode`), with title/caption.
//
// The bench is loaded ssr:false — it is pointer-event driven and client-only,
// the same discipline MechanicsBenchRenderer, MotionLabRenderer,
// VectorBoardRenderer and the simulation registry follow. See
// field-bench/archetypes.ts for the construction library and
// PHYSICS_SIMULATION_PROGRAM.md §4 for the pedagogy.
...
const FieldBench = dynamic(() => import('./field-bench/FieldBench'), {
  ssr: false,
  loading: () => (
    <div
      className="w-full animate-pulse rounded-2xl border"
      style={{ height: 480, background: SIM_SURFACE, borderColor: BORDER.card }}
    />
  ),
});

export default function FieldBenchRenderer({ block }: { block: FieldBenchBlock }) {
