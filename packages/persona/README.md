...
| `writer.ts` | `applyAttemptToProgress` — single mutation surface for the tiered counter contract on `UserProgress`. Mutates an in-memory hydrated mongoose doc; caller saves. Re-exports `resolveConfidenceTier` for back-compat. Thin wrapper that delegates math to `user-progress-updater.ts`. **Canonical mutation surface** ([CRUCIBLE_ARCHITECTURE.md](../../_agents/CRUCIBLE_ARCHITECTURE.md) §9 invariant #3) — do not duplicate this logic elsewhere. |
| `user-progress-updater.ts` | `computeUserProgressUpdate(snapshot, attempt): UserProgressUpdate` — pure value-oriented core of `writer.ts`. Takes a `UserProgressSnapshot` (the value shape of a hydrated `UserProgress`), returns the changes a single attempt would produce. No I/O, no Mongoose, no `'server-only'`. Safe to import from tests, admin simulations, or AI training pipelines replaying historical attempts. The streak-math seam that audit #18 lives on. |
