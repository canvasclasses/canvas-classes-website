...
const p9 = {
  ...
  blocks: [
    ...
    b('worked_example', 9, {
      label: 'how slowly does the magnet really fall?',
      variant: 'solved_example',
      reveal_mode: 'tap_to_reveal',
      problem: 'Put numbers on the demonstration this page opened with. A small cylindrical magnet of mass $ 10 $ g is dropped down a vertical copper pipe of inner radius $ a = 7.0 $ mm and wall thickness $ t = 1.5 $ mm. Take the strength of its field where it meets the pipe wall as $ B \\approx 0.20 $ T, the resistivity of copper as $ \\rho = 1.7\\times10^{-8}\\ \\Omega\\cdot\\text{m} $, and $ g = 10\\ \\text{m/s}^{2} $. Estimate the terminal velocity, and the time to fall through $ 1.0 $ m of pipe. Compare it with a free fall through the same metre.',
      solution: '**Read this as an estimate, not as a formula to memorise.** The magnet\'s field is not uniform and the pipe is not a set of separate rings, so we are going to make some deliberate simplifications and say what they are. What survives is the **scaling** — which quantities matter, and how strongly.\n\n**The three assumptions, stated before any algebra.**\n\n1. Treat the pipe as a stack of independent rings. In the middle of the magnet the flux through a ring hardly changes as the magnet slides past, because the field there is much the same from one millimetre to the next. **The flux only changes near the magnet\'s leading and trailing faces**, so we count two short **active regions**, each of axial length $ \\ell $.\n2. Take $ \\ell \\approx a $, the pipe radius — the distance over which a magnet\'s field dies away sideways. This is the shakiest assumption of the three, and happily $ \\ell $ **cancels out**, which is why the estimate is steadier than it looks.\n3. Use the single number $ B $ for both the flux through a ring and the sideways field that pushes on it. In reality those two differ; this is where the factor-of-two uncertainty lives.\n\n**Step 1 — the emf in one active ring.**\n\nAs the magnet advances by $ \\ell $, the flux through a ring there goes from about zero to about $ B\\pi a^{2} $. So\n\n$ \\varepsilon \\approx \\frac{B\\pi a^{2}}{\\ell}\\,v $\n\nThis is nothing but $ \\varepsilon = -\\frac{d\\Phi}{dt} $ written as $ \\frac{d\\Phi}{dz}\\times v $, with the sign handled by Lenz\'s law: the current always opposes the magnet.\n\n**Step 2 — the resistance of that ring.**\n\nA ring of radius $ a $, height $ \\ell $ and wall thickness $ t $ has length $ 2\\pi a $ round and cross-section $ t\\ell $:\n\n$ R = \\frac{\\rho\\,(2\\pi a)}{t\\ell} $\n\n**Step 3 — the current, where $ \\ell $ leaves the problem.**\n\n$ I = \\frac{\\varepsilon}{R} = \\frac{B\\pi a^{2}v}{\\ell}\\times\\frac{t\\ell}{2\\pi\\rho a} = \\frac{B\\,a\\,t\\,v}{2\\rho} $\n\n**Step 4 — the retarding force.**\n\nThe ring carries $ I $ round a circumference $ 2\\pi a $ in the magnet\'s field, so $ F = BI(2\\pi a) $ per active region, and there are two of them:\n\n$ F \\approx 2\\times B\\left(\\frac{Batv}{2\\rho}\\right)(2\\pi a) = \\frac{2\\pi a^{2} t B^{2}}{\\rho}\\,v $\n\nLook at the shape of that: force proportional to $ v $, with $ B^{2} $ on top and a resistivity underneath. It is page 8\'s $ F = \\frac{B^{2}l^{2}v}{R} $ wearing the geometry of a pipe.\n\n**Step 5 — terminal velocity.**\n\nSet the retarding force equal to the weight:\n\n$ mg = \\frac{2\\pi a^{2} t B^{2}}{\\rho}v_{T} \\qquad\\Rightarrow\\qquad v_{T} = \\frac{mg\\rho}{2\\pi a^{2} t B^{2}} $\n\nNumerator: $ mg\\rho = (0.010)(10)(1.7\\times10^{-8}) = 1.7\\times10^{-9} $.\n\nDenominator: $ 2\\pi(7.0\\times10^{-3})^{2}(1.5\\times10^{-3})(0.20)^{2} = 2\\pi(4.9\\times10^{-5})(1.5\\times10^{-3})(0.040) = 1.85\\times10^{-8} $.\n\n$ v_{T} = \\frac{1.7\\times10^{-9}}{1.85\\times10^{-8}} = 0.092\\ \\text{m/s} \\approx 9\\ \\text{cm/s} $\n\n**Step 6 — the two times.**\n\nThrough a metre of pipe: $ t = \\frac{1.0}{0.092} \\approx 11\\ \\text{s} $.\n\nIn free fall through the same metre: $ t = \\sqrt{\\frac{2h}{g}} = \\sqrt{0.20} = 0.45\\ \\text{s} $.\n\nAbout **twenty-five times slower** — which is exactly the "several seconds" the demonstration shows, and the reason it looks like a trick.\n\n**And the energy, in one line.** At terminal velocity the kinetic energy has stopped changing, so every joule of gravitational potential energy goes straight into heating the pipe: $ P = mgv_{T} = (0.10)(0.092) \\approx 9\\ \\text{mW} $, and over the whole metre $ Q = mgh = 0.10 $ J. Small, but it is all there is — nothing else happened.\n\n**What to trust here.** The number is good to a factor of two or three, no better. What is solid is the scaling $ v_{T} \\propto \\frac{\\rho}{B^{2}t} $: swap copper for aluminium and $ \\rho $ rises by about $ 60\\% $, so the magnet falls about $ 60\\% $ faster; halve the wall thickness and it falls twice as fast; use a magnet half as strong and it falls **four** times as fast. That last one is why the demonstration needs a strong magnet and a thick pipe.',
    }),
    b('callout', 10, {
      variant: 'real_world',
