/**
 * Zod-validate every page of the Class 12 Physics book straight from the DB.
 * Run: npx tsx scripts/physics12-book/_validate.mts
 */
...
(async () => {
  await withDb(async (db: any) => {
    ...
    for (const p of pages.sort((a: any, b: any) => a.chapter_number - b.chapter_number || a.page_number - b.page_number)) {
      ...
      if (r.success) console.log(`OK   ch${p.chapter_number} p${String(p.page_number).padStart(2)} ${p.slug}`);
