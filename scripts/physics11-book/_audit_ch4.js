'use strict';
/**
 * Read-only audit of Class 11 Physics Ch.4 "Laws of Motion". Same metrics as
 * _audit_ch3.js — answer-position balance, length-tells, difficulty coverage,
 * action-gated ratio, exposition-run cap, per-page practice counts.
 *
 * Run: node scripts/physics11-book/_audit_ch4.js
 */
...
withDb(async (db) => {
  ...
  const pages = await db.collection('book_pages')
    .find({ book_id: book._id, chapter_number: 4, deleted_at: null }).toArray();
  ...
  for (const p of pages) {
    // The per-page floor (4 step-solvers, 5 quiz Qs) is a CONTENT-page rule.
    // A chapter opener is a cover; a recap / exercises / drill page is a
    // close-out whose whole job is practice_bank items, not new teaching.
    // Flagging those made the real signal harder to read, so exempt them from
    // the floor — they are still audited for MCQ hygiene and prose runs.
    const isOpener = p.page_type === 'chapter_opener'
      || /-(recap|ncert-exercises|jee-neet-drill)$/.test(p.slug);
  ...
  console.log('length-tells (correct option is the longest):', lengthTell,
    `(${mcqTotal ? ((lengthTell / mcqTotal) * 100).toFixed(0) : 0}%)`);
