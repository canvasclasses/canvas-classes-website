'use strict';
/**
 * Shared scaffold for the Class 11 Mathematics Live Book — Chapter 1 "Sets".
 * Forked from `_book.js` (Chapter 2's scaffold) so both chapters can be built
 * concurrently without touching a shared file. PURELY ADDITIVE + idempotent
 * (matches by slug). Everything is created UNPUBLISHED (is_published/published
 * = false) — nothing reaches students until the founder reviews + publishes in
 * the admin books editor.
 *
 * Book: "Class 11 Mathematics" (subject: mathematics, grade 11, board ncert)
 * Chapter 1: "Sets" (NCERT Ch.1 numbering kept, so Ch.2 Relations & Functions /
 * Ch.3 Trig keep their own numbers).
 */
...
const CH = {
  ...
  slug: 'sets',
  description:
    'Well-defined collections and how to describe them; empty, finite, infinite and equal sets; ' +
    'subsets and intervals on the real line; Venn diagrams; union, intersection, difference and ' +
    'complement; De Morgan’s laws; and counting problems with two sets.',
};
