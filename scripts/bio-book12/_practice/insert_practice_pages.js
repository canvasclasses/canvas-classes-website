'use strict';
/**
 * Inserts the per-chapter "Practice — NCERT Exercises" pages into class12-biology.
 *
 * ADDITIVE ONLY. Each chapter gets ONE new page appended as its last page; the
 * 108 existing pages are never touched (no re-save, no version churn). Re-running
 * is idempotent: if a chapter's practice page already exists (by slug), it is
 * UPDATED in place via book-writer.savePage (versioned, content-loss-guarded)
 * rather than duplicated.
 *
 * Reads page modules from scripts/bio-book12/_practice/ch<N>.js (one per chapter,
 * authored by subagents against ./CONTRACT.md).
 *
 * Dry-run by default; pass --apply to write.
 * Usage: node scripts/bio-book12/_practice/insert_practice_pages.js [--apply]
 */
...
async function main() {
  ...
  try {
    ...
    for (const { chapter, mod } of mods) {
      ...
      if (existing) {
        // update the blocks + metadata through the guarded gateway
        if (APPLY) {
          ...
          // ensure it is wired into the chapter (in case it wasn't)
        ...
        // brand-new page appended after the current last page
