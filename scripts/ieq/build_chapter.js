// Orchestrator for the Ionic Equilibrium (Ch.7) page build. Same pattern as the
// Chemical Equilibrium build; reuses the shared helpers. Globs scripts/ieq/pages/*.js
// (each exports {page_number, slug, title, subtitle, page_type, build(h)}), builds
// each with fresh helpers, upserts to ncert-simplified chapter 7 (insert new /
// update-blocks if slug exists), links new pages into chapters[7].page_ids.
// published:false. Dry-run by default; pass --apply to write. One DB connection,
// sequential, no races.
//   node scripts/ieq/build_chapter.js            # dry-run
//   node scripts/ieq/build_chapter.js --apply    # write
