// Build the Class 12 Ch.4 p-Block Elements end-of-chapter Practice page.
//   ./p_pblock.js → the practice_bank page (NCERT exercises + Exemplar, 4 group sections)
// Targets book `ncert-simplified-12`, chapter number 4. published:false.
// Standalone & additive — creates/updates ONLY the p-block-practice page; never
// touches the 22 existing content pages. Dry-run by default; pass --apply.
require('dotenv').config({ path: '.env.local' });
const { MongoClient } = require('mongodb');
const { makeHelpers, uuidv4 } = require('../ceq/_helpers');

const APPLY = process.argv.includes('--apply');
const m = require('./p_pblock.js');
...
(async () => {
  ...
  let _id;
  const common = { title: m.title, subtitle: m.subtitle, page_type: 'lesson', tags: m.tags || [], content_types: ['practice_bank'], reading_time_min: 25, blocks, updated_at: new Date() };
  if (existing) {
