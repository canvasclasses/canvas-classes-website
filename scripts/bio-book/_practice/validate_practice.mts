...
// A practice page legitimately ENDS on a practice_bank (not an inline_quiz),
// so that structural rule is not applied. Mirrors bio-book12/_practice exactly.
let bad = 0;
for (let ch = 1; ch <= 19; ch++) {
  const f = join(process.cwd(), `scripts/bio-book/_practice/ch${ch}.js`);
