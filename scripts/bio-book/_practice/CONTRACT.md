# Class 11 Biology — NCERT Exercises page authoring contract

You are authoring the **"Practice — NCERT Exercises"** page for ONE chapter of the Class 11
...
3. **The SOLUTION is yours, and it must be excellent + NCERT-faithful.** Full, plain,
   ...
   and the final ratio/answer. Everything must be consistent with NCERT Class 11 Biology — do not
...
Every `id` (blocks, sections, items) must be a **unique uuid v4 string, hardcoded as a literal
string in the source** (e.g. `id: 'b224f104-d662-4b83-9a0f-fdf6dc3810fe'`) — generate it once
(e.g. `node -e "console.log(require('crypto').randomUUID())"`) and paste the value in.
**Never call a uuid-generating function inline at module load time** (e.g.
`const { randomUUID: uuid } = require('crypto'); ...; id: uuid()`) — a real bug hit exactly this
on 2026-07-30: every `require()` of the module produced fresh random ids, so
`insert_practice_pages.js`'s idempotent "page already exists → UPDATE" path saw the entire old
block set as "removed" and the entire new set as "added," tripping book-writer's content-loss
guard and crashing the insert loop mid-chapter (which also left already-inserted earlier
chapters' pages un-wired into `chapters[].page_ids`, since that write happens once at the end of
the whole loop). Static, hardcoded ids make re-running the insert script against an unchanged
module a true no-op, which is what makes it safe to run repeatedly. Block `order` is 0,1,2.
**The page ends on the practice_bank — do NOT add an inline_quiz** (this page type is the
drill itself).
