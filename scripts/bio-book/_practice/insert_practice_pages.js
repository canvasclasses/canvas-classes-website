'use strict';
/**
 * Inserts the per-chapter "Practice — NCERT Exercises" pages into class11-biology.
 *
 * ADDITIVE ONLY. Each chapter gets ONE new page appended as its last page; existing
 * pages are never touched (no re-save, no version churn). Re-running is idempotent:
 * if a chapter's practice page already exists (by slug), it is UPDATED in place via
 * book-writer.savePage (versioned, content-loss-guarded) rather than duplicated.
 *
 * Reads page modules from scripts/bio-book/_practice/ch<N>.js (one per chapter,
 * authored against ./CONTRACT.md). Mirrors scripts/bio-book12/_practice/ exactly.
 *
 * Dry-run by default; pass --apply to write.
 * Usage: node scripts/bio-book/_practice/insert_practice_pages.js [--apply]
 */
