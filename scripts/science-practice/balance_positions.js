 ...
 * science chapter_practice bank, so "always pick B" can never beat the quiz.
 *
 * Reads scripts/science-practice/_ch<N>_practice.json, reorders each question's
 ...
 *
 *   node scripts/science-practice/balance_positions.js <ch> [--write]
 *
 * NOTE: assumes every question has 4 options, and that explanations reference
 * option CONTENT, not a letter/position — verify that before trusting the result.
 */
const fs = require('fs');
...
const path = `scripts/science-practice/_ch${ch}_practice.json`;
